...
library(purrr)
```
`dictr` provides a new data structure specialized for representing dictionary-like
data structures, and several functions to transform them.
...
* they will do *partial matching* by default (`my_dict$co` will also return `blue`, since it partially matches the key `color`, instead of throwing an error or returning NULL);
...
Unlike other packages, `dict`s are regular value types that use lists under the hood.
...
You can get keys and values using the `keys` and `values` functions (`keys` is
a synonym for `names`):
```{r, eval=FALSE}
...
- `has(dict, key)` returns TRUE if `dict` contains `key` (equivalent to `key %in% keys(dict)`).
...
- `rename_keys(dict, new_key=old_key, ...)` will rename `old_key` to `new_key`
...
## Strict dictionaries
You can create a strict dictionary using `strict_dict`. Any time a non-existing key is accessed, an exception is thrown using `stop()` instead of returning `NULL`.
